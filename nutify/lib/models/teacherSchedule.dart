...
class TeacherSchedule {
  ...
  static Future<List<TeacherSchedule>> getTeacherSchedulesByDay(String dayOfWeek) async {
    try {
      ...
      const String baseUrl = 'https://nutify.x10.mx/api.php';
      
      final response = await http.post(
        Uri.parse(baseUrl),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'action': 'getTeacherSchedulesByDay',
          'teacher_id': userId,
          'day_of_week': dayOfWeek,
        }),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData is List) {
          return responseData.map((json) => TeacherSchedule.fromJson(json)).toList();
        } else if (responseData is Map && responseData['schedules'] != null) {
