<?php
// Updated API functions for Student Inbox with real database integration

function getStudentInboxPending($conn, $userID) {
    try {
...
function getStudentInboxCompleted($conn, $userID) {
    try {
...
function getStudentInboxMissed($conn, $userID) {
    try {
...
function getStudentInboxCancelled($conn, $userID) {
    try {
...
// Updated studentGrabTeacherList function with user_dept integration
function studentGrabTeacherList($conn) {
    try {
        // Join users with user_dept to get real department information
        $sql = "SELECT 
                    u.user_id, 
                    u.user_fn, 
                    u.user_ln,
                    CONCAT(u.user_fn, ' ', u.user_ln) as full_name,
                    COALESCE(ud.department, 'No Department Assigned') as department
                FROM users u 
                LEFT JOIN user_dept ud ON u.user_id = ud.user_id 
                WHERE u.user_type = 'teacher' 
                ORDER BY u.user_fn, u.user_ln";
...
// Example of how to integrate these functions into your existing api.php
...
case 'getStudentInboxPending':
    ...
    if (isset($input['userID'])) {
        getStudentInboxPending($conn, $input['userID']);
    } else {
        echo json_encode(['success' => false, 'error' => true, 'message' => 'Missing userID parameter']);
    }
    break;
...
case 'getStudentInboxCompleted':
    $input = json_decode(file_get_contents('php://input'), true);
    if (isset($input['userID'])) {
        getStudentInboxCompleted($conn, $input['userID']);
    } else {
        echo json_encode(['success' => false, 'error' => true, 'message' => 'Missing userID parameter']);
    }
    break;
...
case 'getStudentInboxMissed':
    $input = json_decode(file_get_contents('php://input'), true);
    if (isset($input['userID'])) {
        getStudentInboxMissed($conn, $input['userID']);
    } else {
        echo json_encode(['success' => false, 'error' => true, 'message' => 'Missing userID parameter']);
    }
    break;
...
case 'getStudentInboxCancelled':
    $input = json_decode(file_get_contents('php://input'), true);
    if (isset($input['userID'])) {
        getStudentInboxCancelled($conn, $input['userID']);
    } else {
        echo json_encode(['success' => false, 'error' => true, 'message' => 'Missing userID parameter']);
    }
    break;
